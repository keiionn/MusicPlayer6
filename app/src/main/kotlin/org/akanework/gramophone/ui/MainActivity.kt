...
import org.akanework.gramophone.logic.utils.SongDeletionHelper
...
class MainActivity : AppCompatActivity() {
    ...
    lateinit var deleteIntentSender: ActivityResultLauncher<IntentSenderRequest>
        private set
    var pendingDeletion: Pair<MediaItem, (Boolean) -> Unit>? = null
    /**
     * updateLibrary:
    ...
    private fun registerDeleteIntentSender() {
        deleteIntentSender =
            registerForActivityResult(ActivityResultContracts.StartIntentSenderForResult()) { result ->
                if (result.resultCode == RESULT_OK) {
                    pendingDeletion?.let { (item, callback) ->
                        SongDeletionHelper.tryDelete(this, item, callback,wasAuthorized = true)
                        pendingDeletion = null
                    } ?: Toast.makeText(this, getString(R.string.delete_in_progress), Toast.LENGTH_SHORT).show()
                } else {
                    pendingDeletion = null
                }
            }
