        ...
        popupMenu.setOnMenuItemClickListener { it1 ->
            when (it1.itemId) {
                ...
                R.id.delete -> {
                    ...
                    SongDeletionHelper.confirmAndDelete(
                        context = fragment.requireContext(),
                        launcher = activity.deleteIntentSender,
                        item = item
                    ) { success ->
                        if (success) Toast.makeText(context, "删除成功", Toast.LENGTH_SHORT).show()
                        else Toast.makeText(context, "删除失败或取消", Toast.LENGTH_SHORT).show()
                    }
                    true
                }
